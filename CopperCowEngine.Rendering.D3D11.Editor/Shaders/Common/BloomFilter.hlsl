...
void VerticalFilterPass(uint3 Gid : SV_GroupID, uint GI : SV_GroupIndex)
{
    int2 coord = int2(Gid.x, GI - kernelhalf + (groupthreads - kernelhalf * 2) * Gid.y);
    coord = clamp(coord, int2(0, 0), int2(cbDownScale.Res.x - 1, cbDownScale.Res.y - 1));
    ...
    if (GI >= kernelhalf && GI < (groupthreads - kernelhalf) 
        && ((GI - kernelhalf + (groupthreads - kernelhalf * 2) * Gid.y) < cbDownScale.Res.y))
    {
        float4 vOut = 0;
...
[numthreads(groupthreads, 1, 1)]
void HorizontalFilterPass(uint3 Gid : SV_GroupID, uint GI : SV_GroupIndex)
{
    ...
    // Horizontal blur   
    if (GI >= kernelhalf && GI < (groupthreads - kernelhalf) 
        && ((Gid.x * (groupthreads - 2 * kernelhalf) + GI - kernelhalf) < cbDownScale.Res.x))    
    {        
        float4 vOut = 0;  
