...
namespace EngineCore.Technique
{
    class DefferedQuadRenderer
    {
        ...
        public DefferedQuadRenderer(Vector2 pos, Vector2 scale, Quaternion rot, int Type) {
            ...
            ShaderBytecodePack m_ShaderBytecodePack = AssetsLoader.GetShaderPack("DefferedPBRQuadShader");

            m_VertexShader = new VertexShader(
                Engine.Instance.Device,
                m_ShaderBytecodePack.VertexShaderByteCode
            );
            m_PixelShader = new PixelShader(
                Engine.Instance.Device,
                m_ShaderBytecodePack.PixelShaderByteCode
            );
            m_InputLayout = new InputLayout(
                Engine.Instance.Device,
                ShaderSignature.GetInputSignature(m_ShaderBytecodePack.VertexShaderByteCode),
                new[] {
        ...
        private Vector3 GetRandomRay()
        {
            float theta, cosphi, sinphi;
            theta = 2.0f * MathUtil.Pi * RandomUtil.NextFloat(randGen, 0.0f, 1.0f);
            cosphi = 1.0f - 2.0f * RandomUtil.NextFloat(randGen, 0.0f, 1.0f);
            sinphi = (float)System.Math.Sqrt(1 - System.Math.Min(1.0, System.Math.Pow(cosphi, 2f)));
           
            return new Vector3(sinphi * (float)System.Math.Cos(theta), sinphi * (float)System.Math.Sin(theta), cosphi);
        }
        ...
        public void Draw(GBuffer gBuffer) {
            ...
            m_ConstantBuffer.CameraPosition = new Vector4(Engine.Instance.MainCamera.gameObject.transform.Position, 0);

            if (Engine.Instance.MainLight != null)
            {
                m_LightBuffer[0].viewProjMatrix = Engine.Instance.MainLight.ViewProjMatrix;
                m_LightBuffer[0].lightTint = Engine.Instance.MainLight.ambientColor;
