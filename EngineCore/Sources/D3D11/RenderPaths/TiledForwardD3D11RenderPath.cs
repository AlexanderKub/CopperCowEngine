...
namespace EngineCore.D3D11
{
    internal class TiledForwardD3D11RenderPath : BaseD3D11RenderPath
    {
        ...
        private uint DirLightsCount = 0;
        private uint NonDirLightsCount = 0;
        private void UpdateLights(List<StandardFrameData.LightData> lights)
        {
            ...
            DirLightsCount = 0;
            NonDirLightsCount = 0;
            foreach (StandardFrameData.LightData light in lights)
            {
                if (light.Type == LightType.Directional) {
                    if (DirLightsCount < 3)
                    {
                        m_DirLightsConstBuffer[DirLightsCount] = new CommonStructs.ConstBufferDirLightStruct()
                        {
