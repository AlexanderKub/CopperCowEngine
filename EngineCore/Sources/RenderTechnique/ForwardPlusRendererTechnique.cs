...
namespace EngineCore.RenderTechnique
{
    internal class ForwardPlusRendererTechnique : BaseRendererTechnique
    {
        ...
        private void UpdateLights()
        {
            ...
            for (int i = 0; i < lights.Length; i++)
            {
                NonDirLightCenterAndRadiusArray[i] = new Vector4(lights[i].gameObject.transform.WorldPosition, lights[i].radius);
            ...
            return;
            t += Engine.Instance.Time.DeltaTime * 0.5f;
           
            for (int i = 0; i < 256; i++)
            {
                float angle = t % (MathUtil.Pi * 2f) + (i % 44) * halfDeg - (i / 44) * halfDeg ;
                float cos = (float)System.Math.Cos(angle);
                float sin = (float)System.Math.Sin(angle);

                float x = (i % 16) / 15.0f - 0.5f;
                float z = (i / 16) / 15.0f - 0.5f;
                float dx = x * cos - z * sin;
                float dz = x * sin + z * cos;
                NonDirLightCenterAndRadiusArray[i] = new Vector4(dx * 120f, NonDirLightCenterAndRadiusArray[i].Y, dz * 120f, NonDirLightCenterAndRadiusArray[i].W);
            }
        ...
        private void InitLightsData()
        {
            ...
            float range = 120f;
            float fRadius = 0.075f * range * RandomUtil.NextFloat(random, 1.0f, 5.0f);
            fRadius = 15f;
            float CosOfCone = (float)System.Math.Cos(MathUtil.DegreesToRadians(45));

            for (int i = 0; i < MAX_NUM_LIGHTS; i++)
            {
                NonDirLightCenterAndRadiusArray[i] = new Vector4(RandomUtil.NextFloat(random, -range * 0.5f, range * 0.5f),
                    RandomUtil.NextFloat(random, 0, 5.0f), RandomUtil.NextFloat(random, -range * 0.5f, range * 0.5f), fRadius);

                NonDirLightColorArray[i] = new Vector4(RandomUtil.NextVector3(random, Vector3.Zero, Vector3.One), 1.0f);

                Vector3 dir = RandomUtil.NextVector3(random, -Vector3.One, Vector3.One);
                CosOfCone = (float)System.Math.Cos(MathUtil.DegreesToRadians(RandomUtil.NextFloat(random, 30, 80)));
                dir.Z = dir.Z > 0 ? CosOfCone : -CosOfCone;
                NonDirLightParamsArray[i] = new Vector4(dir, 0.0f);
            }*/
