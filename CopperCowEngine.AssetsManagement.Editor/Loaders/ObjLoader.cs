...
namespace CopperCowEngine.AssetsManagement.Editor.Loaders
{
    internal static class ObjLoader
    {
        ...
        private class GeometryBuffer
        {
            ...
            private bool IsEmpty => _vertices.Count == 0;
            ...
            public ModelGeometry GetGeometry()
            {
                ...
                foreach (var fi in od.AllFaces)
                {
                    ...
                    if (!verticesDictionary.TryGetValue(newKey, out var vNum))
                    {
                        ...
                        vNum = k;
                        tmpVertices.Add(_vertices[fi.Vi]);
                        if (HasNormals)
                        {
                            tmpNormals.Add(_normals[fi.Vn]);
                        }
                        if (HasUVs)
                        {
                            tmpUVs.Add(_uvs[fi.Vu]);
                        }
                        k++;
                    }
        ...
        private static readonly Dictionary<string, ModelGeometry> CachedModels = new Dictionary<string, ModelGeometry>();
